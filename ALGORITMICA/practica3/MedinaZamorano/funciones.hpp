...
void minimoMaximo(vector<T> &v, int n, int i, int j, int &max, int &min, int &llamadas){
	...
	if (estaOrdenado(v)==true) //Caso de que el vector esta ordenado
	{
		max=v[j];
		min=v[i];
	}else{
		if(i==j){ //Tiene un elemento
			max = v[i];
			min = v[i];
		}else{
			if(i ==(j-1)){ //Tiene solo 2 elementos
				if(v[i]<v[j]){
					max = v[j];
					min = v[i];
				}else{
			...
			}else{ //Caso general cuando tiene 3 o mas elementos
				mitad = (i+j)/2;
				llamadas++;
